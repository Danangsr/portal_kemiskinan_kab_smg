...
router.use(function timeLog(req, res, next) {
  ...
  //connection.end();
   ...
   router.get('/desil', function(req, res){
 //connection.connect();
 //console.log(req.query)
 let tahun =2023
 if(req.query.tahun){
 ...
 let max =0;
  connection.query("SELECT count(*) as jml, asWkt(a.SHAPE) as geometry, a.wadmkc as kecamatan, a.wadmkd as desa, a.id_des_kel FROM admin_kab_smg a left join kemiskinan b on a.id_des_kel= b.Id_des_kel where b.desil_kesejahteraan="+desil+" and b.tahun_data="+tahun+" group by a.id_des_kel order by jml desc", function(err, rows, fields) {
    if (err) throw err;
 max = rows[0].jml;
 for (let i = 0; i < rows.length; i++) {
  rows[i].persen = (rows[i].jml * 100)/max;
...
//SELECT count(*) as jml, asWkt(a.SHAPE) as geometry, a.wadmkc as kecamatan, a.wadmkd as desa, a.id_des_kel FROM admin_kab_smg a left join kemiskinan b on a.id_des_kel= b.Id_des_kel where b.desil_kesejahteraan=1 and b.tahun_data=2023 group by a.id_des_kel order by jml desc
 
router.get('/topojson_kec', function(req, res){
  //connection.connect();
  //console.log(req.query)
  connection.query("SELECT asWkt(a.SHAPE) as geometry, a.wadmkc as kecamatan, a.id FROM admin_kec a ", function(err, rows, fields) {
    if (err) throw err;
