...
passport.use(new LocalStrategy({
  ...
  function(req, username, password, done) {
    connection.query("SELECT u.*,r.role,r.role_id from user u left join role r on r.role_id = u.role WHERE username='"+username+"' and u.deleted = 0 and r.login = 1" , function(err, rows, fields) {
      if (err) throw err;
   
     console.log(rows)
    if(rows.length >0){
    	rows[0].tujuan = req.body.tujuan;
      console.log(rows[0].pwd != sha1(password),rows[0].pwd , sha1(password));
...
router.post('/login_user',
  passport.authenticate('local', { 
    ...
    if (req.user[0].role_id == 2) {
      console.log(req.user[0].role_id);
      res.redirect('/dashboard_user')
    }else if (req.user[0].role_id == 4) {
      res.redirect('/dashboard_user/camat')
    }else if (req.user[0].role_id == 5) {
      res.redirect('/dashboard_user/lurah')
    }else{
      res.redirect('/dashboard_user/opd')}
...
module.exports.cek_login_depan = function(req, res, next) {
	...
	if (req.isAuthenticated() && req.user[0].google != 1){
        ...
        res.redirect('/login_user?tujuan='+encodeURIComponent(req.originalUrl));
    }
}
